...
Both variables are required. A Hetzner Cloud API token is not an object-storage
access/secret key pair. Use the bucket's S3 credentials. The worker needs permission
...
unavailable. Credentials are not accepted as integration config keys.
