...
Keep working locally while the worker runs that saved version. Its selected outputs go to S3, ready to download or use in another task.

Your config supplies the Git remote, storage destination, and files to save, so you can reuse those choices across tasks. Passwords and keys stay in environment variables.

## Read more

- [Connect your database, Git, and storage](doc/configuration.md)
- [Choose Git and S3 for each task](doc/integrations.md)
- [Keep and reuse output files](doc/s3.md)
