...
Create a new task with JSON data.

```bash
nextask add PATH --data JSON [--status STATUS]
```

**Arguments:**
- `PATH` - Task path (e.g., `/experiments/run1`)

**Options:**
- `--data JSON` - JSON data for the task (required)
- `--status TEXT` - Initial status (default: pending)

**Example:**
```bash
nextask add /experiments/ppo/run1 --data '{"lr": 0.001, "epochs": 100}'
```
...
```bash
nextask list [--prefix PREFIX] [--status STATUS] [--limit N] [--json]
```
...
```bash
nextask update PATH [--status STATUS] [--data JSON]
```
...
**Options:**
- `--status TEXT` - New status (pending/running/completed/failed)
- `--data JSON` - JSON data to merge with existing data
...
```bash
nextask stats [--prefix PREFIX]
```

**Options:**
- `--prefix TEXT` - Show stats for specific prefix (default: /)
...
```bash
nextask clear [--prefix PREFIX] [--status STATUS] [--all] [--yes]
```

**Options:**
- `--prefix TEXT` - Clear tasks with specific prefix
- `--status TEXT` - Clear tasks with specific status
- `--all` - Clear all tasks
- `--yes` - Skip confirmation prompt

**Example:**
```bash
nextask clear --status failed --yes
```
...
Start a local Redis server managed by nextask.

```bash
nextask redis start [--port PORT] [--name NAME] [--db-path PATH] [--daemonize]
```

**Options:**
- `--port INTEGER` - Port to run on (default: 6379)
- `--name TEXT` - Named instance for easy identification
- `--db-path PATH` - Directory for Redis data files
...
Stop nextask-managed Redis server(s).

```bash
nextask redis stop [--name NAME] [--port PORT] [--all]
```

**Options:**
- `--name TEXT` - Stop named instance
- `--port INTEGER` - Stop instance on specific port
- `--all` - Stop all nextask-managed instances
...
# Add tasks
nextask --port 6380 add /exp/run1 --data '{"lr": 0.001}'
nextask --port 6380 add /exp/run2 --data '{"lr": 0.01}'

# List tasks
nextask --port 6380 list

# Update task
nextask --port 6380 update /exp/run1 --status completed

# View statistics
nextask --port 6380 stats
